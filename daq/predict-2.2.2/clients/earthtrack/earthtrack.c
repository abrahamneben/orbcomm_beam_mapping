...
*****************************************************************************
*                                                                           *
*           This code was last modified by KD2BD on 07-Jun-2003.            *
*                                                                           *
*****************************************************************************
*                                                                           *
...
char argc, *argv[];
{
	...
	if (xplanet)
	{
		...
		fprintf(fd,"marker_font=/usr/local/share/xplanet/fonts/FreeSans.ttf\n");
		...
		if (x==2)
		{
			fprintf(stderr,"%c*** Error: Could not read \"/usr/local/shhare/xplanet/config/default\"\nIs xplanet installed?\n\n",7);
			exit(-1);
		}
	}
	...
	while (error==0)
	{
		...
		if (error==0)
		{
			...
			{
				if (circledrawn)
					sprintf(command,"xplanet -config %s -proj orth -longitude %f -latitude %f -radius %d -num_times 1 -starfreq 0.005 %s",configfile,mapcenterlong,mapcenterlat,radius,xtra);
				else
					sprintf(command,"xplanet -config %s -proj orth -latitude %f -longitude %f -num_times 1 %s -starfreq 0.005",configfile,mapcenterlat,mapcenterlong,xtra);
			}
