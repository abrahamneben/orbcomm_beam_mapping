...
*               Copyright John A. Magliacane, KD2BD 1991-2003               *
*                      Project started: 26-May-1991                         *
*                        Last update: 24-Jun-2003                           *
*****************************************************************************
		...
		if (strncmp("GET_SAT",buf,7)==0)
		{
			...
			for (i=0; i<24; i++)
			{
				if ((strcmp(satname,sat[i].name)==0) || (atol(satname)==sat[i].catnum))
				{
					...
					sprintf(buff,"%s\n%-7.2f\n%+-6.2f\n%-7.2f\n%+-6.2f\n%ld\n%-7.2f\n%-7.2f\n%-7.2f\n%-7.2f\n%ld\n%c\n%-7.2f\n%-7.2f\n%-7.2f\n",sat[i].name,long_array[i],lat_array[i],az_array[i],el_array[i],(long)rint(86400.0*(nextevent[i]+3651.0)),footprint_array[i],range_array[i],altitude_array[i],velocity_array[i],orbitnum_array[i],visibility_array[i],phase_array[i],eclipse_depth_array[i],squint_array[i]);
...
void Banner()
{
	...
	mvprintw(6,18,"   KD2BD Software -- Copyright 1991-2003   ");
	mvprintw(7,18,"                                           ");
}
...
void SaveQTH()
{
	/* This function saves QTH data file normally
	   found under $HOME/.predict/predict.qth */
...
char mode;
{
	...
	if (AosHappens(indx) && Geostationary(indx)==0 && Decayed(indx,daynum)==0)
	{
		...
		do
		{
			...
			{
				if (calc_squint)
					sprintf(string,"      %s%4d %4d  %4d  %4d   %4d   %6ld  %4.0f %c\n",Daynum2String(daynum),iel,iaz,ma256,isplat,isplong,irk,squint,findsun);
				else
					sprintf(string,"      %s%4d %4d  %4d  %4d   %4d   %6ld  %6ld %c\n",Daynum2String(daynum),iel,iaz,ma256,isplat,isplong,irk,rv,findsun);
...
void QthEdit()
{
	...
	mvprintw(12,44,"%g",qth.stnlat);
	mvprintw(13,44,"%g",qth.stnlong);
...
char speak;
{
	...
	int	ans, oldaz=0, oldel=0, length,
		xponder=0, polarity=0, tshift, bshift;
	char	approaching=0, command[80], comsat, aos_alarm=0,
		los_alarm=0, geostationary=0, aoshappens=0,
		decayed=0, eclipse_alarm=0, sun_alarm=0, visibility=0;
	double	oldtime=0.0, nextaos=0.0, lostime=0.0, aoslos=0.0,
		...
		dopp, doppler100=0.0, delay, loss, shift,
	 	old_eclipse_depth=0.0, eclipse_threshold; 
	...
	eclipse_threshold=deg2rad*(-0.5);  /* eclipse depth of -0.5 degrees */
	...
	mvprintw(8+tshift,1,"       W.            El           km            km          km              km");
	...
	do
	{
		...
		if (sat_lat>=0.0)
		{
			mvprintw(7+tshift,1,"%-6.2f",sat_lat);
			attrset(COLOR_PAIR(4)|A_BOLD);
			mvprintw(7+tshift,8,"N");
		}
		...
		mvprintw(8+tshift,1,"%-7.2f",360.0-sat_lon);
		...
		if (sat_ele>=0.0)
		{
			...
			{
				if (fabs(eclipse_depth)<0.015) /* ~1 deg */
				{
					/* Hold off regular announcements if
					   eclipse depth is within +/- 1 deg. */

					oldtime=CurrentDaynum();

					if ((eclipse_alarm==0) && (old_eclipse_depth<0.0) && (eclipse_depth>eclipse_threshold))
					{
					...
					if ((sun_alarm==0) && (old_eclipse_depth>0.0) && (eclipse_depth<eclipse_threshold))
					{
				...
				if ((CurrentDaynum()-oldtime)>(0.00003*sqrt(sat_alt)))
				{
					...
					system(command);
  					oldtime=CurrentDaynum();
					old_eclipse_depth=eclipse_depth;
				}
	...
	do
	{
		for (z=0; z<24; z++)
		{
			...
			if (sat[indx].meanmo!=0.0 && Decayed(indx,0.0)!=1)
			{
				...
				mvprintw(y+6,x,"%-10s%3.0f  %+3.0f  %3.0f   %3.0f %6.0f %c", Abbreviate(sat[indx].name,9),sat_azi,sat_ele,sat_lat,360.0-sat_lon,sat_range,sunstat);
...
void MainMenu()
{
	...
	mvprintw(16,1,"[L]: Lunar Pass Predictions");
	mvprintw(17,1,"[O]: Solar Pass Predictions");
...
char argc, *argv[];
{
	...
	/* Set up default qth, tle, and dat filenames and paths */

	sprintf(qthfile,"%s/.predict/predict.qth",getenv("HOME"));
	sprintf(tlefile,"%s/.predict/predict.tle",getenv("HOME"));
	sprintf(dbfile,"%s/.predict/predict.db",getenv("HOME"));

	/* If the predict.qth and predict.tle files do not
	   exist under ~/.predict, then we have a new user. */
	...
	/* Handle the absence of a predict.db file and scan for
	   optional command line arguments if not a new user */

	if (newuser==0)
	{
		/* The database is new to version 2.2.x.  If such a file
		   doesn't already exist under ~/.predict, place a default
		   copy from the PREDICT distribution under ~/.predict. */
