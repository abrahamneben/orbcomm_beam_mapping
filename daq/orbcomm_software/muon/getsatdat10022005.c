...
{
  int bin=0, k;
  double freqbins[(N/2)+1];

     for (k=0; k<(N/2)+1; k++)
	{
	  freqbins[k]=(sqrt(freqdata[k][0]*freqdata[k][0]+freqdata[k][1]*freqdata[k][1])/256);
	} 
  ...
  cpgmove(bin, freqbins[0]);
  for (bin=1; bin<(N/2)+1; bin++)
	 {
	   cpgdraw(bin, freqbins[bin]);
	 }
  return 0;
}
