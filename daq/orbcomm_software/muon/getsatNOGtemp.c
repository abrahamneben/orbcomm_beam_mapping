...
F64 accumFreqData[515]; 
F64 accumFreqData2[515];
F64 accumFreqData3[515];
F64 accumFreqData4[515];
...
int main(void)
{
  ...
  */
     if ((fp1=fopen(acqname,"r"))==NULL)
	{
     ...
     timedata = fftw_malloc(sizeof(double) * (N+2));
     freqdata = fftw_malloc(sizeof(fftw_complex) * (N+2));

     timedata2 = fftw_malloc(sizeof(double) * (N+2));
     freqdata2 = fftw_malloc(sizeof(fftw_complex) * (N+2));

     timedata3 = fftw_malloc(sizeof(double) * (N+2));
     freqdata3 = fftw_malloc(sizeof(fftw_complex) * (N+2));

     timedata4 = fftw_malloc(sizeof(double) * (N+2));
     freqdata4 = fftw_malloc(sizeof(fftw_complex) * (N+2));

      read_count=N*M;
      //      mem_size=read_count * (2*(channel+1));
      mem_size=read_count * (2*(channel+2));
	  ...
	  for (Mcount=1; Mcount<=M; Mcount++)
	    {
	      ...
	      p2=fftw_plan_dft_r2c_1d(N, timedata2, freqdata2, FFTW_FORWARD);
	      p3=fftw_plan_dft_r2c_1d(N, timedata3, freqdata3, FFTW_FORWARD);
	      p4=fftw_plan_dft_r2c_1d(N, timedata4, freqdata4, FFTW_FORWARD);
...
int getSatInfo1()
{
	...
	satnum=22; //SET TO MAXIMUM NUMBER OF SATELLITES
