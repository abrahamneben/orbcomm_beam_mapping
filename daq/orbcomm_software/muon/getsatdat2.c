...
double *timedata, *timedata2;
fftw_complex *freqdata, *freqdata2;
fftw_plan p, p2;
//int N=512;
int N=1024;
  //  int M=50;
...
int main(void)
{
  ...
  float accumFreqData[512]; 
  float accumFreqData2[512]; 
		...
		for (kk=0; kk<(N/2); kk++)
		  {
		    ...
		    accumFreqData[kk]+=(sqrt(freqdata[kk][0]*freqdata[kk][0]+freqdata[kk][1]*freqdata[kk][1])/(sqrt(2)*512*M));	  
		    accumFreqData2[kk]+=(sqrt(freqdata2[kk][0]*freqdata2[kk][0]+freqdata2[kk][1]*freqdata2[kk][1])/(sqrt(2)*512*M));	  
		  } 
	      }
	    fprintf(fp, "Bin Voltages \n   CH 1     CH 2\n");

	    for (jj=0; jj<(N/2); jj++) fprintf(fp, "%f  %f\n", accumFreqData[jj], accumFreqData2[jj]);
...
void transfer_and_scale(U16 *bufA, int num_chan )
{
  ...
  for(i=0; i<read_count; i++)
    ...
    j+=num_chan;
    //    printf("%i  %f  %f %i\n",i, voltarray[0][i], voltarray[1][i], read_count);
...
int postfft()
{
  ...
  fftw_free(timedata2); fftw_free(freqdata2);
 return 0;
}    
