function plot_slice_null2(tiledata,sliceangle,plottitle,nsides,counts,pol,refmodelboresightgain) % pol is 'ew' or 'ns'
    ...
    plot(xsort,10.^(.1*data(xsortind)),[col,'.'],'MarkerSize',12);hold on;
    %plot(modelzasort,model(modelzasortind),[col,'-']);
    title(plottitle);xlim([-80 80]);ylim([0 1]);
    xlabel('\theta');
    ylabel('dB','rot',0);
    set(gca,'linewidth',3);
