function [tilebeamew,tilebeamns,tilebeamewstd,tilebeamnsstd,tilebeamnumpasses,modeltilebeamew,modeltilebeamns,counts,outliers,refpowhpixew,refpowhpixns,refmodelhpixewlin,refmodelhpixnslin]=make_tilebeams(tileew,refew,tilens,refns,refmodelew,refmodelns,pixnums,nsides,delaynums,passnum,satnum)
    ...
    for pixi=1:npix
        ...
        modeltilebeamew(pixi) = 10*log10((1-sin(pixza)^2*sin(pixaz)^2)*array_factor_and_ground_screen_power_beam(pixza,pixaz,delaynums));
        modeltilebeamns(pixi) = 10*log10((1-sin(pixza)^2*cos(pixaz)^2)*array_factor_and_ground_screen_power_beam(pixza,pixaz,delaynums));
        ...
        %disp(length(pixeldataew));
        %disp(['pixi=',num2str(pixi)]);
%         if mod(pixi,20)==0
%             figure;
%             hist(pixeldatans,linspace(0,100,100));xlim([0 100]);pause(1);
%         end
        ...
        %disp(mean(pixeldataew));
        tilebeamns(pixi) = 10*log10(mean(pixeldatans));
        %tilebeamew(pixi) = mean(pixeldataew); disp('WARNING, TILE BEAMS NOT BEING COMPUTED IN dB');
        ...
        %if pixi==20
        %    figure;
        %    hist(pixeldataew,100);
        %end
    end

    tilebeamew(isnan(tilebeamew))=0;
    tilebeamns(isnan(tilebeamns))=0;
