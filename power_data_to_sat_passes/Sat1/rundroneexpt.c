...
void show_channel_data(U16 *bufA, int num_chan );
int plot_channel_data(void);
int transfer_and_scale(void);
...
int main(void)
{
  ...
  int plan;
  int k, kk;
  double accumFreqData[515]; 
  double accumFreqData2[515];
  double accumFreqData3[515];
  double accumFreqData4[515];
  double gpstime;
...
/*       freqdata4 = fftw_malloc(sizeof(fftw_complex) * (N+1)); */
   ...
   for (acqcycles=0; acqcycles<acqmax; acqcycles++)
      ...
      if ((fp=fopen(fname,"a"))==NULL)
	{
      ...
      //      ai_buf = (U16*)calloc(mem_size, sizeof(U16));
      ...
      for (j=1; j<=max_scans; j++)
	{
	  ...
	  for (ind=0; ind<N; ind++) timedata[ind]=0;
	  for (ind=0; ind<N; ind++) timedata2[ind]=0;
	  for (ind=0; ind<N; ind++) timedata3[ind]=0;
	  for (ind=0; ind<N; ind++) timedata4[ind]=0;


	  //	  for (Mcount=1; Mcount<=M; Mcount++)
	  for (Mcount=0; Mcount<M; Mcount++)
	    {
	      for (ind=0; ind<N; ind++) timedata[ind]=voltarray[0][ind+(N*(Mcount))];
	      for (ind=0; ind<N; ind++) timedata2[ind]=voltarray[1][ind+(N*(Mcount))];
	      for (ind=0; ind<N; ind++) timedata3[ind]=voltarray[2][ind+(N*(Mcount))];
	      for (ind=0; ind<N; ind++) timedata4[ind]=voltarray[3][ind+(N*(Mcount))];

	      plan=(j==1)&&(Mcount==0);
	      if (plan) p=fftw_plan_dft_r2c_1d(N, timedata, freqdata, FFTW_MEASURE);
	      if (plan) p2=fftw_plan_dft_r2c_1d(N, timedata2, freqdata2, FFTW_MEASURE);
	      if (plan) p3=fftw_plan_dft_r2c_1d(N, timedata3, freqdata3, FFTW_MEASURE);
	      if (plan) p4=fftw_plan_dft_r2c_1d(N, timedata4, freqdata4, FFTW_MEASURE);
	  ...
	  fprintf(fp,"\n%.3f ",gpstime);
	  for (jj=satchan-dchans; jj<satchan+dchans; jj++) fprintf(fp, "%7.6f ", accumFreqData[jj]);
	  for (jj=satchan-dchans; jj<satchan+dchans; jj++) fprintf(fp, "%7.6f ", accumFreqData2[jj]);
	  for (jj=satchan-dchans; jj<satchan+dchans; jj++) fprintf(fp, "%7.6f ", accumFreqData3[jj]);
	  for (jj=satchan-dchans; jj<satchan+dchans; jj++) fprintf(fp, "%7.6f ", accumFreqData4[jj]);
...
int acquire_data()
{
    ...
    printf("%ld data samples acquired.\n", read_count);
    return 0;
}

int transfer_and_scale()
{
  ...
  num_chan=channel+1;
  //  bufA = (U16*)calloc(mem_size, sizeof(U16));
  //
  ...
  //  free(bufA);
    printf("Data transfered and scaled.\n");
    return 0;
} 
